...
const openVPNReadyMarker = "Initialization Sequence Completed"
...
func (b *OpenVPNBackend) Activate(ctx context.Context, profilePath, providerDir string) error {
	...
	command := exec.Command(b.opts.Command, "--config", profilePath)
...
func (b *OpenVPNBackend) startFailure(proc *openVPNProcess, reason string) error {
	...
	if cleanupErr != nil {
		return fmt.Errorf("%s; cleanup failed: %w", reason, cleanupErr)
	}
...
func (b *OpenVPNBackend) drainDiagnostics(proc *openVPNProcess) {
	...
	tail := make([]byte, 0, len(openVPNReadyMarker)-1)
	for {
		count, err := proc.reader.Read(buffer)
		if count > 0 {
			chunk := buffer[:count]
			combined := append(append([]byte(nil), tail...), chunk...)
			if bytes.Contains(combined, []byte(openVPNReadyMarker)) {
				proc.readyOnce.Do(func() { close(proc.ready) })
			}
			keep := len(openVPNReadyMarker) - 1
...
func redactOpenVPNDiagnostic(message string) string {
	...
	for _, sensitive := range []string{"password", "passwd", "auth", "private key", "pre-shared key", "token", "credential", "secret", "certificate", "cert", "pkcs12", "tls-crypt", "tls-auth", "crl-verify"} {
