...
func (i OpenVPNConfigInspector) Inspect(profilePath, profileDir string) error {
	...
	if !pathWithin(profileDir, profilePath) {
		return errors.New("OpenVPN profile is outside its profile directory")
	}
	if err := validatePrivateDirectory(profileDir); err != nil {
		return fmt.Errorf("validate OpenVPN profile directory: %w", err)
	}
	...
	for {
		...
		if continued {
			...
			if err := i.inspectLogicalLine(logical, profileDir); err != nil {
				return fmt.Errorf("OpenVPN config line %d: %w", line, err)
			}
			logical = ""
		}
...
func (i OpenVPNConfigInspector) inspectLogicalLine(line, profileDir string) error {
	...
	if !filepath.IsAbs(secretPath) {
		secretPath = filepath.Join(profileDir, secretPath)
	}
	secretPath = filepath.Clean(secretPath)
	if !pathWithin(profileDir, secretPath) {
		return fmt.Errorf("directive %q references a file outside its profile directory", directive)
	}
	if err := validateConfinedPrivateFile(profileDir, secretPath); err != nil {
